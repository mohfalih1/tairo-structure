...
const mainClass = computed(() => {
  ...
  if (!collapseEnabled.value) {
    return 'bg-muted-100 dark:bg-muted-900 relative min-h-screen w-full overflow-x-hidden px-10 transition-all duration-300'
  }
  //background layout
  const list = [
    'bg-[#101010] relative min-h-screen w-full overflow-x-hidden px-10 transition-all duration-300',
  ]

  if (isOpen.value) {
    list.push('lg:max-w-[calc(100%_-210px)] lg:ms-[210px]')
  }
  else {
